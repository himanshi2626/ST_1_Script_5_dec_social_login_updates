$(document).ready(function() {var formatter = new CucumberHTML.DOMFormatter($('.cucumber-report'));formatter.uri("src/test/java/Features/1Facebook_existing_user_sign_in.feature");
formatter.feature({
  "line": 2,
  "name": "Sign in with Facebook",
  "description": "",
  "id": "sign-in-with-facebook",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@facebook_existing_user_login"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign in with Facebook",
  "description": "",
  "id": "sign-in-with-facebook;test-sign-in-with-facebook",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on Home Page old fb",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "User click on sign in with facebook button old fb",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user go to free ppts page old fb",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user download a free ppt old fb",
  "keyword": "Then "
});
formatter.match({
  "location": "Facebook_existing_user_sign_in.user_is_already_on_Home_Page_old_fb()"
});
formatter.result({
  "duration": 4189836600,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_existing_user_sign_in.user_click_on_sign_in_with_facebook_button_old_fb()"
});
formatter.result({
  "duration": 27616884900,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_existing_user_sign_in.user_go_to_free_ppts_page_old_fb()"
});
formatter.result({
  "duration": 7751031500,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_existing_user_sign_in.user_download_a_free_ppt_old_fb()"
});
formatter.result({
  "duration": 26393634200,
  "status": "passed"
});
formatter.uri("src/test/java/Features/2facebook_logout.feature");
formatter.feature({
  "line": 1,
  "name": "Logout the appliction .",
  "description": "",
  "id": "logout-the-appliction-.",
  "keyword": "Feature"
});
formatter.scenario({
  "line": 2,
  "name": "log out the Fb paid user account.",
  "description": "",
  "id": "logout-the-appliction-.;log-out-the-fb-paid-user-account.",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 3,
  "name": "navigate to the facebook url.",
  "keyword": "Given "
});
formatter.match({
  "location": "facebook_logout.navigate_to_the_facebook_url()"
});
formatter.result({
  "duration": 19253890200,
  "status": "passed"
});
formatter.uri("src/test/java/Features/3Google_existing_user_sign_in.feature");
formatter.feature({
  "line": 2,
  "name": "Sign in with Google",
  "description": "",
  "id": "sign-in-with-google",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@google_existing_user_login"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign in with Google",
  "description": "",
  "id": "sign-in-with-google;test-sign-in-with-google",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on Home Page old gmail",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "User click on sign in with google plus button old gmail",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user go to free ppts page old gmail",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user download a free ppt old gmail",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "logout pld gmail",
  "keyword": "Then "
});
formatter.match({
  "location": "Google_existing_user_sign_in.user_is_already_on_Home_Page_old_gmail()"
});
formatter.result({
  "duration": 9097424400,
  "status": "passed"
});
formatter.match({
  "location": "Google_existing_user_sign_in.user_click_on_sign_in_with_google_plus_button_old_gmail()"
});
formatter.result({
  "duration": 18566724100,
  "status": "passed"
});
formatter.match({
  "location": "Google_existing_user_sign_in.user_go_to_free_ppts_page_old_gmail()"
});
formatter.result({
  "duration": 9611590100,
  "status": "passed"
});
formatter.match({
  "location": "Google_existing_user_sign_in.user_download_a_free_ppt_old_gmail()"
});
formatter.result({
  "duration": 9824477900,
  "status": "passed"
});
formatter.match({
  "location": "Google_existing_user_sign_in.logout_pld_gmail()"
});
formatter.result({
  "duration": 16723455900,
  "status": "passed"
});
formatter.uri("src/test/java/Features/4Google_plus_user_logout.feature");
formatter.feature({
  "line": 1,
  "name": "Clear google plus history.",
  "description": "",
  "id": "clear-google-plus-history.",
  "keyword": "Feature"
});
formatter.scenario({
  "line": 2,
  "name": "Redirect to google plus page.",
  "description": "",
  "id": "clear-google-plus-history.;redirect-to-google-plus-page.",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 3,
  "name": "open the google plus page.",
  "keyword": "Given "
});
formatter.match({
  "location": "Google_plus_user_logout.open_the_google_plus_page()"
});
formatter.result({
  "duration": 25834405300,
  "status": "passed"
});
formatter.uri("src/test/java/Features/5Facebook_new_account_sign_in.feature");
formatter.feature({
  "line": 2,
  "name": "Sign up with Facebook",
  "description": "",
  "id": "sign-up-with-facebook",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@facebook_new_account_sign_in"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign up with Facebook",
  "description": "",
  "id": "sign-up-with-facebook;test-sign-up-with-facebook",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on Home Page new fb",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "User click on sign in with facebook button",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user go to free ppts page new fb",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user download a free ppt new fb",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user delete the fb account new fb",
  "keyword": "Then "
});
formatter.match({
  "location": "Facebook_new_account_sign_in.user_is_already_on_Home_Page_new_fb()"
});
formatter.result({
  "duration": 7914367500,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_new_account_sign_in.user_click_on_sign_in_with_facebook_button()"
});
formatter.result({
  "duration": 20537187800,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_new_account_sign_in.user_go_to_free_ppts_page_new_fb()"
});
formatter.result({
  "duration": 2659650800,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_new_account_sign_in.user_download_a_free_ppt_new_fb()"
});
formatter.result({
  "duration": 8810653500,
  "status": "passed"
});
formatter.match({
  "location": "Facebook_new_account_sign_in.user_delete_the_fb_account_new_fb()"
});
formatter.result({
  "duration": 21040778800,
  "status": "passed"
});
formatter.uri("src/test/java/Features/6Google_new_user_sign_up.feature");
formatter.feature({
  "line": 2,
  "name": "Sign up with Google",
  "description": "",
  "id": "sign-up-with-google",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@NewUserlogin_with_GPlus"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign up with Google",
  "description": "",
  "id": "sign-up-with-google;test-sign-up-with-google",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on Home Page new google",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "User click on sign in with google button",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user go to free ppts page new google",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user download a free ppt new google",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user delete the google account new google",
  "keyword": "Then "
});
formatter.match({
  "location": "Google_new_user_sign_up.user_is_already_on_Home_Page_new_google()"
});
formatter.result({
  "duration": 7288567800,
  "status": "passed"
});
formatter.match({
  "location": "Google_new_user_sign_up.user_click_on_sign_in_with_google_button()"
});
formatter.result({
  "duration": 19752247100,
  "status": "passed"
});
formatter.match({
  "location": "Google_new_user_sign_up.user_go_to_free_ppts_page_new_google()"
});
formatter.result({
  "duration": 14498841400,
  "status": "passed"
});
formatter.match({
  "location": "Google_new_user_sign_up.user_download_a_free_ppt_new_google()"
});
formatter.result({
  "duration": 10886992100,
  "status": "passed"
});
formatter.match({
  "location": "Google_new_user_sign_up.user_delete_the_fb_account_new_google()"
});
formatter.result({
  "duration": 20970783000,
  "status": "passed"
});
formatter.uri("src/test/java/Features/7Sign_in_existing_free_user.feature");
formatter.feature({
  "line": 2,
  "name": "Sign in with existing free user",
  "description": "",
  "id": "sign-in-with-existing-free-user",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@sign_in_existing_free_user"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign in existing free user",
  "description": "",
  "id": "sign-in-with-existing-free-user;test-sign-in-existing-free-user",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on sign in page efu",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "user enter email and password efu",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user click on login button efu",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user go to free ppts page efu",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user download a free ppt",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user logout efu.",
  "keyword": "Then "
});
formatter.match({
  "location": "Sign_in_existing_free_user.user_is_already_on_sign_in_page_efu()"
});
formatter.result({
  "duration": 5635475100,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_free_user.user_enter_email_and_password_efu()"
});
formatter.result({
  "duration": 5199317700,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_free_user.user_click_on_login_button_efu()"
});
formatter.result({
  "duration": 2619690800,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_free_user.user_go_to_free_ppts_page_efu()"
});
formatter.result({
  "duration": 4581750500,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_free_user.user_download_a_free_ppt()"
});
formatter.result({
  "duration": 9033071400,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_free_user.user_logout_efu()"
});
formatter.result({
  "duration": 16690434600,
  "status": "passed"
});
formatter.uri("src/test/java/Features/8sign_up_correct_data.feature");
formatter.feature({
  "line": 2,
  "name": "Sign up with correct data",
  "description": "",
  "id": "sign-up-with-correct-data",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@Sign_Up_Correct_Data"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign up with correct Data",
  "description": "",
  "id": "sign-up-with-correct-data;test-sign-up-with-correct-data",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on sign up page cd",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "enter new email to sign up cd",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "User enter firstname and lastname to sign up cd",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user enter password and confirm password to sign up cd",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user enter captcha to sign up cd",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "check checkbox to sign up cd",
  "keyword": "Then "
});
formatter.step({
  "line": 11,
  "name": "user click on sign up button to sign up cd",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "user lands on pricing page and then user go to free ppts page cd",
  "keyword": "Then "
});
formatter.step({
  "line": 13,
  "name": "user download a free product cd",
  "keyword": "Then "
});
formatter.step({
  "line": 14,
  "name": "user delete the new account created cd",
  "keyword": "Then "
});
formatter.match({
  "location": "sign_up_correct_data.user_is_already_on_sign_up_page_cd()"
});
formatter.result({
  "duration": 9611896900,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.enter_new_email_to_sign_up_cd()"
});
formatter.result({
  "duration": 6249824900,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_enter_firstname_and_lastname_to_sign_up_cd()"
});
formatter.result({
  "duration": 8222218900,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_enter_password_and_confirm_password_to_sign_up_cd()"
});
formatter.result({
  "duration": 8312312600,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_enter_captcha_to_sign_up_cd()"
});
formatter.result({
  "duration": 2115731700,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.check_checkbox_to_sign_up_cd()"
});
formatter.result({
  "duration": 2000404900,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_click_on_sign_up_button_to_sign_up_cd()"
});
formatter.result({
  "duration": 4157885300,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_lands_on_pricing_page_and_then_user_go_to_free_ppts_page_cd()"
});
formatter.result({
  "duration": 6773206100,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_download_a_free_product_cd()"
});
formatter.result({
  "duration": 10304435400,
  "status": "passed"
});
formatter.match({
  "location": "sign_up_correct_data.user_delete_the_new_account_created_cd()"
});
formatter.result({
  "duration": 22404503400,
  "status": "passed"
});
formatter.uri("src/test/java/Features/9Sign_in_existing_paid_user.feature");
formatter.feature({
  "line": 2,
  "name": "Sign in with existing paid user",
  "description": "",
  "id": "sign-in-with-existing-paid-user",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@9_sign_in_existing_paid_user"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign in existing paid user",
  "description": "",
  "id": "sign-in-with-existing-paid-user;test-sign-in-existing-paid-user",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on sign in page epu",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "user enter email and password epu",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user click on login button epu",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user navigate to complete deck from account dashboard page",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user download any product",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "User Logout.",
  "keyword": "Then "
});
formatter.match({
  "location": "Sign_in_existing_paid_user.user_is_already_on_sign_in_page_epu()"
});
formatter.result({
  "duration": 15119462400,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_paid_user.user_enter_email_and_password_epu()"
});
formatter.result({
  "duration": 218120700,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_paid_user.user_click_on_login_button_epu()"
});
formatter.result({
  "duration": 2129910400,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_paid_user.user_navigate_to_complete_deck_from_account_dashboard_page()"
});
formatter.result({
  "duration": 8881472500,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_paid_user.user_download_any_product()"
});
formatter.result({
  "duration": 5298334200,
  "status": "passed"
});
formatter.match({
  "location": "Sign_in_existing_paid_user.user_Logout()"
});
formatter.result({
  "duration": 8195637000,
  "status": "passed"
});
formatter.uri("src/test/java/Features/CO_checkout.feature");
formatter.feature({
  "line": 2,
  "name": "2CO Checkout",
  "description": "",
  "id": "2co-checkout",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@2CO_checkout"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test 2CO checkout",
  "description": "",
  "id": "2co-checkout;test-2co-checkout",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on Website Home Page 2CO",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "user navigates to sign up page 2CO",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user create a new ac count 2CO",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user is redirected to pricing page and choose a plan to pay 2CO",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user is redirected to checkout page 2CO",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user proceed to pay with 2CO 2CO",
  "keyword": "Then "
});
formatter.step({
  "line": 11,
  "name": "paypal popup appears and user navigates back to my account 2CO",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "user deleted the account 2CO",
  "keyword": "Then "
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 37
    }
  ],
  "location": "co_checkout.user_is_already_on_Website_Home_Page_CO(int)"
});
formatter.result({
  "duration": 13726626900,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 31
    }
  ],
  "location": "co_checkout.user_navigates_to_sign_up_page_CO(int)"
});
formatter.result({
  "duration": 6196754400,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 27
    }
  ],
  "location": "co_checkout.user_create_a_new_ac_count_CO(int)"
});
formatter.result({
  "duration": 34484470700,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 60
    }
  ],
  "location": "co_checkout.user_is_redirected_to_pricing_page_and_choose_a_plan_to_pay_CO(int)"
});
formatter.result({
  "duration": 9134918500,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 36
    }
  ],
  "location": "co_checkout.user_is_redirected_to_checkout_page_CO(int)"
});
formatter.result({
  "duration": 2999880000,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 25
    },
    {
      "val": "2",
      "offset": 29
    }
  ],
  "location": "co_checkout.user_proceed_to_pay_with_CO_CO(int,int)"
});
formatter.result({
  "duration": 13584400400,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 59
    }
  ],
  "location": "co_checkout.paypal_popup_appears_and_user_navigates_back_to_my_account_CO(int)"
});
formatter.result({
  "duration": 10205314500,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "2",
      "offset": 25
    }
  ],
  "location": "co_checkout.user_deleted_the_account_CO(int)"
});
formatter.result({
  "duration": 28500195600,
  "status": "passed"
});
formatter.uri("src/test/java/Features/Sign_up_incorrect_data.feature");
formatter.feature({
  "line": 2,
  "name": "Sign up with incorrect data",
  "description": "",
  "id": "sign-up-with-incorrect-data",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@Sign_up_Incorrect_data"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test Sign up with Incorrect Data",
  "description": "",
  "id": "sign-up-with-incorrect-data;test-sign-up-with-incorrect-data",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on sign up page",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "user enter incorrect data in the sign sig up form",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "form is error message is displayed and form is not submitted",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "enter new email to sign up",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "User enter firstname and lastname to sign up",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user enter password and confirm password to sign up",
  "keyword": "Then "
});
formatter.step({
  "line": 11,
  "name": "user enter captcha to sign up",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "check checkbox to sign up",
  "keyword": "Then "
});
formatter.step({
  "line": 13,
  "name": "user click on sign up button to sign up",
  "keyword": "Then "
});
formatter.step({
  "line": 14,
  "name": "user is on pricing page",
  "keyword": "Then "
});
formatter.step({
  "line": 15,
  "name": "user go to free ppts page",
  "keyword": "Then "
});
formatter.step({
  "line": 16,
  "name": "user download a free product",
  "keyword": "Then "
});
formatter.step({
  "line": 17,
  "name": "user delete the new account created",
  "keyword": "Then "
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_is_already_on_sign_up_page()"
});
formatter.result({
  "duration": 14760049000,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_enter_incorrect_data_in_the_sign_sig_up_form()"
});
formatter.result({
  "duration": 28784885600,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.form_is_error_message_is_displayed_and_form_is_not_submitted()"
});
formatter.result({
  "duration": 22286727200,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.enter_new_email_to_sign_up()"
});
formatter.result({
  "duration": 6482054300,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_enter_firstname_and_lastname_to_sign_up()"
});
formatter.result({
  "duration": 8180073700,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_enter_password_and_confirm_password_to_sign_up()"
});
formatter.result({
  "duration": 8248932900,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_enter_captcha_to_sign_up()"
});
formatter.result({
  "duration": 4108394700,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.check_checkbox_to_sign_up()"
});
formatter.result({
  "duration": 2000123900,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_click_on_sign_up_button_to_sign_up()"
});
formatter.result({
  "duration": 5262840300,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_is_on_pricing_page()"
});
formatter.result({
  "duration": 2999867800,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_go_to_free_ppts_page()"
});
formatter.result({
  "duration": 4687806900,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_download_a_free_product()"
});
formatter.result({
  "duration": 6753483800,
  "status": "passed"
});
formatter.match({
  "location": "Sign_up_incorrect_data.user_delete_the_new_account_created()"
});
formatter.result({
  "duration": 25355487900,
  "status": "passed"
});
formatter.uri("src/test/java/Features/paypal_checkout.feature");
formatter.feature({
  "line": 2,
  "name": "Paypal Checkout",
  "description": "",
  "id": "paypal-checkout",
  "keyword": "Feature",
  "tags": [
    {
      "line": 1,
      "name": "@paypal_checkout"
    }
  ]
});
formatter.scenario({
  "line": 3,
  "name": "Test paypal checkout",
  "description": "",
  "id": "paypal-checkout;test-paypal-checkout",
  "type": "scenario",
  "keyword": "Scenario"
});
formatter.step({
  "line": 5,
  "name": "user is already on Website Home Page pp",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "user navigates to sign up page pp",
  "keyword": "Then "
});
formatter.step({
  "line": 7,
  "name": "user create a new ac count pp",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "user is redirected to pricing page and choose a plan to pay pp",
  "keyword": "Then "
});
formatter.step({
  "line": 9,
  "name": "user is redirected to checkout page pp",
  "keyword": "Then "
});
formatter.step({
  "line": 10,
  "name": "user proceed to pay with paypal pp",
  "keyword": "Then "
});
formatter.step({
  "line": 11,
  "name": "paypal popup appears and user navigates back to my account pp",
  "keyword": "Then "
});
formatter.step({
  "line": 12,
  "name": "user deleted the account pp",
  "keyword": "Then "
});
formatter.match({
  "location": "paypal_checkout.user_is_already_on_Website_Home_Page_pp()"
});
formatter.result({
  "duration": 13644896200,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.user_navigates_to_sign_up_page_pp()"
});
formatter.result({
  "duration": 7161568600,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.user_create_a_new_ac_count_pp()"
});
formatter.result({
  "duration": 34520668900,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.user_is_redirected_to_pricing_page_and_choose_a_plan_to_pay_pp()"
});
formatter.result({
  "duration": 8969297100,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.user_is_redirected_to_checkout_page_pp()"
});
formatter.result({
  "duration": 8086085600,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.user_proceed_to_pay_with_paypal_pp()"
});
formatter.result({
  "duration": 8168438300,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.paypal_popup_appears_and_user_navigates_back_to_my_account_pp()"
});
formatter.result({
  "duration": 14426480700,
  "status": "passed"
});
formatter.match({
  "location": "paypal_checkout.user_deleted_the_account_pp()"
});
formatter.result({
  "duration": 24381753900,
  "status": "passed"
});
});